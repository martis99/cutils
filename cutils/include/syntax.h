...
#define STX_TERM_RULE(_rule)                              \
	(stx_term_data_t)                                 \
	{                                                 \
		.type = STX_TERM_RULE, .val.rule = _rule, \
	}

#define STX_TERM_TOKEN(_token)                               \
	(stx_term_data_t)                                    \
	{                                                    \
		.type = STX_TERM_TOKEN, .val.token = _token, \
	}

#define STX_TERM_LITERAL(_literal)                                 \
	(stx_term_data_t)                                          \
	{                                                          \
		.type = STX_TERM_LITERAL, .val.literal = _literal, \
	}

#define STX_TERM_OR(_l, _r)                                         \
	(stx_term_data_t)                                           \
	{                                                           \
		.type = STX_TERM_OR, .val.orv = {.l = _l, .r = _r } \
	}
